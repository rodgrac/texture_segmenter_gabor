...
def reduced_image(img_bank):
    ...
    for i in range(3):
        # temp = ((img_tensor[:, :, i].T - np.mean(img_tensor[:, :, i].T, axis=0)) / (
        #     np.std(img_tensor[:, :, i].T, axis=0))).T
        temp = img_tensor[:, :, i]
...
if __name__ == "__main__":
    ...
    freq, theta = createbankParams(1 / (2 + (2 * np.sqrt(np.log(2)) / np.pi)), 5, 8)
    ...
    output_g = gaussian_smoothing(output_ne, img_s, 5, 2)
    output_ng = ((output_g.T - np.min(output_g.T, axis=0)) / (
            (np.max(output_g.T, axis=0)) - (np.min(output_g.T, axis=0))))

    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 1.0)
    ret, label, center = cv2.kmeans(np.float32(output_ng), 2, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)

    # color = np.random.randn(K, 3)
    if (sum(label) / label.size) > 0.5:
